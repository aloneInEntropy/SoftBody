...
bool SkinnedMesh::loadMesh(std::string mesh_name, bool popBuffers) {
    populateBuffer = popBuffers;
    mesh_path = mesh_name;

    std::string rpath = MODELPATH(mesh_name) + mesh_name;
...
bool SkinnedMesh::initScene(const aiScene* scene, std::string mesh_name) {
...
void SkinnedMesh::initSingleMesh(unsigned int mIndex, const aiMesh* amesh) {
...
int SkinnedMesh::createAnimationList(const aiNode* node, int childIdx, mat4 parent) {
...
std::vector<mat4> SkinnedMesh::getBoneTransforms(float timeSinceStarted, float animSpeed) {
...
std::vector<mat4> SkinnedMesh::loadAnimation(float aTime) {
...
const aiNodeAnim* SkinnedMesh::findNodeAnim(const aiAnimation* anim, const std::string nodeName) {
...
bool SkinnedMesh::initMaterials(const aiScene* scene, std::string mesh_name) {
    std::string dir = MODELPATH(mesh_name);
    bool flag = true;
...
bool SkinnedMesh::loadDiffuseTexture(const aiMaterial* pMaterial, std::string dir, unsigned int index) {
...
bool SkinnedMesh::loadSpecularTexture(const aiMaterial* pMaterial, std::string dir, unsigned int index) {
...
void SkinnedMesh::render(unsigned int nInstances, const mat4* bone_trans_matrix, const float* depths) {
...
void SkinnedMesh::render(unsigned int nInstances, const mat4* bone_trans_matrix) {
    std::vector<float> dpths(nInstances, 0);
    render(nInstances, bone_trans_matrix, dpths.data());
}

void SkinnedMesh::render(mat4 mm) {
    render(1, &mm);
}

std::vector<mat4> SkinnedMesh::getUpdatedTransforms(Shader* skinnedShader, float animSpeed) {
    return getBoneTransforms(SM::getGlobalTime(), animSpeed);
}

std::vector<mat4> SkinnedMesh::getUpdatedTransforms(float animSpeed) {
    return getUpdatedTransforms(shader, animSpeed);
}

void SkinnedMesh::update(Shader* skinnedShader, float animSpeed) {
...
void SkinnedMesh::update(Shader* skinnedShader) {
    update(skinnedShader, 1);
}

void SkinnedMesh::update(float animSpeed) {
