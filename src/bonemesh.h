...
class SkinnedMesh : public Mesh {
   public:
    SkinnedMesh() { name = "NewBoneMesh" + std::to_string(SM::unnamedBoneMeshCount++); }
    SkinnedMesh(std::string nm) { name = nm; }
    SkinnedMesh(std::string nm, std::string mesh_path) {
        name = nm;
        if (!loadMesh(mesh_path, true)) std::cout << "\nfailed to load mesh \"" << nm.c_str() << "\" :(\n\n";
    }
    SkinnedMesh(std::string nm, std::string mesh_path, Shader* s) {
        name = nm;
        if (!loadMesh(mesh_path, true)) std::cout << "\nfailed to load mesh \"" << nm.c_str() << "\" :(\n\n";
        shader = s;
    }
    SkinnedMesh(std::string nm, std::string _mesh_path, int _atlasTileSize, int _atlasTilesUsed, bool load = true) {
        name = nm;
    ...
    SkinnedMesh(std::string nm, std::string _mesh_path, Shader* s, int _atlasTileSize, int _atlasTilesUsed, bool load = true) {
        name = nm;
        shader = s;
        mesh_path = _mesh_path;
        atlasTileSize = _atlasTileSize;
