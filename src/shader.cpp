...
GLuint Shader::CompileComputeShaderGroup(std::vector<std::string> pCSs) {
    // Start the process of setting up our shaders by creating a program ID
    // Note: we will link all the shaders together into this ID
    GLuint shaderProgramID = glCreateProgram();
    if (shaderProgramID == 0) {
    ...
    // Create the compute shader objects
    for (const auto& s : pCSs) {
        std::string pcs = Util::readFile(s.c_str());
        AddShader(shaderProgramID, pcs.c_str(), GL_COMPUTE_SHADER);
    }
