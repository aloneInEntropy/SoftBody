...
class Shader {
   public:
    ...
    // Create a set of attached compute shaders
    Shader(std::string shader_name, std::vector<std::string> compute_shader_paths) {
        name = shader_name;
        ID = CompileComputeShaderGroup(compute_shader_paths);
    }
    ...
    GLuint CompileComputeShaderGroup(std::vector<std::string> pCSs);
